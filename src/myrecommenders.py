...
"""
    ...
    def __init__(self, data, item_features, weighting=True):
        ...
        if weighting:
            self.user_item_matrix_weighted = bm25_weight(self.user_item_matrix.T).T 
        
        self.model = self.fit(self.user_item_matrix_weighted)
...
"""
        ...
        similar_users = [item[0] for item in self.model.similar_users(self.userid_to_id[user], N=N+1)]
        res = []
    ...
    def get_als_recommendations(self, user, N=5):
        ...
        recs = [self.id_to_itemid[rec[0]] for rec in self.model.recommend(userid=self.userid_to_id[user], user_items=csr_matrix(self.user_item_matrix_weighted).tocsr(), N=N, 
                                                                            filter_already_liked_items=False, 
                                                                            filter_items=[self.itemid_to_id[999999]],
                                                                            recalculate_user=False)]
        return recs
    ...
    def get_own_recommendations(self, user, N=5):
        ...
        recs = [self.id_to_itemid[rec[0]] for rec in self.own_recommender.recommend(userid=self.userid_to_id[user], user_items=csr_matrix(self.user_item_matrix).tocsr(), N=N, 
                                                                                    filter_already_liked_items=False, 
                                                                                    filter_items=[self.itemid_to_id[999999]],
                                                                                    recalculate_user=False)]
        return recs
